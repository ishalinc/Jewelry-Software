...
namespace IshalInc.wJewel.Desktop.Forms
{
    public partial class frmAddSalesmanInventory : Telerik.WinControls.UI.RadForm
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (this.salesmenService.SaveInventory(dtSlsSamp, out error))
            {
                if (string.IsNullOrEmpty(error))
                    Helper.AddKeepRec("Add Salesman Log# " + this.txtLogNo.Text);
            }
            if (string.IsNullOrEmpty(error))
            {
                is_cancelled = false;
        ...
        private void btnClose_Click(object sender, EventArgs e)
        {
            ...
            if (qty != 0)
            {
                DialogResult dialogResult = RadMessageBox.Show("Are you sure you want to cancel the changes?", "Add Salesman Log", MessageBoxButtons.YesNo, RadMessageIcon.Question);
                if (dialogResult == DialogResult.Yes)
                {
